...
public class BookingServiceImpl implements BookingService {
    ...
    public BookingDto create(Integer userId, BookingDtoInput bookingDtoInput) {
        ...
        if (!itemRepository.findById(bookingDtoInput.getItemId()).orElseThrow(() -> new EntityNotFoundException("Не найдена вещь")).getAvailable()) {
            throw new EntityBadRequestException("Вещь недоступна для бронирования");
        }
        bookingDto.setStatus(BookingStatus.WAITING);
        bookingDto.setBooker(UserMapper.toUserDto(userRepository.findById(userId).orElseThrow(() -> new EntityNotFoundException("Не найден пользователь"))));
        Item bookingItem = itemRepository.findById(bookingDtoInput.getItemId()).orElseThrow(() -> new EntityNotFoundException("Не найдена вещь"));
    ...
    public BookingDto update(Integer userId, Integer bookingId, Boolean approved) {
        ...
        Booking booking = bookingRepository.findByOwnerIdAndBookingId(userId, bookingId)
                .orElseThrow(() -> new EntityNotFoundException("У пользователя нет этого бронирования с id = " + bookingId));
        if (approved) {
